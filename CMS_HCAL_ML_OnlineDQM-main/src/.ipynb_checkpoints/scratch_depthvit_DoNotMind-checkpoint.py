...
class EncoderBlock(nn.Module):
    ...
    def __init__(
        ...
        self.self_attention = CrissCrossMultiheadSelfAttention(k_factor, in_channels)
...
class DecoderBlock(nn.Module):
    ...
    def __init__(
        ...
        self.self_attention = CrissCrossMultiheadSelfAttention(k_factor, in_channels)
        self.ln_2 = norm_layer(hidden_dim)
        self.cross_attention = CrissCrossMultiheadCrossAttention(k_factor, in_channels)
        self.dropout = nn.Dropout(dropout)
...
class ReverseOperations(nn.Module):
    def __init__(self, in_channels, k_factor, patch_size, image_size):
        super(ReverseOperations, self).__init__()
        self.in_channels = in_channels
        self.k_factor = k_factor
        ...
        self.conv_transpose = nn.ConvTranspose2d(
            in_channels=in_channels * k_factor,
...
class DepthwiseCrossViTMAE(nn.Module):
    def __init__(
        ...
        self.rev_op = ReverseOperations(in_channels, k_factor, patch_size, image_size)
    ...
    def forward(self, imgs: torch.Tensor):
        ...
        x = F.pad(imgs, (0, 9, 0, 17))
        # print("Shape after padding: ", x.shape)
        x = self._process_input(x)
        # print("Shape after processing input: ", x.shape)
        n = x.shape[0]

        enc_output, mask, ids_restore = self.encoder(x, self.mask_ratio)
        x = self.decoder(x, enc_output, ids_restore)
...
def _vision_transformer(
    ...
    **kwargs: Any,
    ...
    image_size = kwargs.pop("image_size", 81)
...
model = _vision_transformer(patch_size = 9,
    in_channels = 7,
    k_factor = 8,
    num_layers = 1,
    hidden_dim = 56,
...
def load_data(year, data_type: str, temporal: str):
    """
    Specify which year you are working on. Options: [2018, 2022]
    Data_type: Can either be in ["train", "test"]
    Temporal: Can use either temporal (ts) or non-temporal (iid). Options: ["ts", "iid"] 
    """
    
    #Load the dataset
    dataset = rf"{data_path}/HE/he_{data_type}_dataset_{year}/{data_type}_data.npy"
...
#Shape of data should be [# of lumisections, depth, ieta, iphi)
train_dataset = load_data(2018, "train")
train_dataset.shape

#Test dataset
test_dataset = load_data(2018, "test")
test_dataset.shape

def split_data(dataset, batch_size, train_size):
    train_data, val_data = train_test_split(dataset, train_size = train_size, shuffle = False)
...
train_loader, val_loader = split_data(dataset = train_dataset, batch_size = batch_size, train_size = train_size)
test_loader = DataLoader(dataset = test_dataset, batch_size = batch_size, shuffle = False)
device = "cuda"

optimizer = optim.Adam(model.parameters(), lr=0.001)
loss_function = nn.MSELoss()
# scheduler = optim.lr_scheduler.CosineAnnealingWarmRestarts(optimizer, T_0=3, T_mult=1, eta_min=0.0001)

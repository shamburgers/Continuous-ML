...
class DepthwiseCrossViTMAE(nn.Module):
    def __init__(
        ...
        # Compute desired size (make image symmetric and divisible by patch_size)
        desired_size = max(self.image_height, self.image_width)
        desired_size = max(desired_size, self.patch_size*self.patch_size)
        if desired_size % self.patch_size != 0:
            desired_size = ((desired_size // self.patch_size) + 1) * self.patch_size
        self.desired_size = desired_size

        self.image_size = desired_size  
          
        # Calculate padding for height and width
        pad_height = self.desired_size - self.image_height
        pad_width = self.desired_size - self.image_width
        
        pad_top = pad_height // 2                    # 17 // 2 = 8
        pad_bottom = pad_height - pad_top            # 17 - 8 = 9
        pad_left = pad_width // 2                    # 9 // 2 = 4
        pad_right = pad_width - pad_left             # 9 - 4 = 5
        ...
        self.rev_op = ReverseOperations(in_channels, k_factor, patch_size, self.image_size)
    ...
    def forward(self, imgs: torch.Tensor):
        ...
        # Reverse operations to reconstruct the image
        x = self.rev_op(x)

        x = x[:, :, self.pad_top:-self.pad_bottom or None, self.pad_left:-self.pad_right or None]
...
def _vision_transformer(
    ...
    **kwargs: Any,
    ...
    image_size = kwargs.pop("image_size", [64, 72])
